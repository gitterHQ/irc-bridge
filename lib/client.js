...
function Client(socket) {
  ...
  this.socket.on('timeout', this.disconnect.bind(this));
...
Client.prototype.keepAlive = function() {
  ...
  function refreshConnectionTimeout() {
  ...
  this.on('PONG', refreshConnectionTimeout);
};
...
Client.prototype.disconnect = function(opts) {
  ...
  if (opts.notify) {
    ...
    this.send(mask, 'PRIVMSG', 'gitter', ': Server restarting...');
  }
