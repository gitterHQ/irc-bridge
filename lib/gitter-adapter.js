...
Adapter.prototype.listenForOneToOnes = function() {
  ...
  var handleNotification = function(msg) {
    ...
    .then(function(room) {
      ...
      var mask = ':' + room.name + '!' + room.name + '@irc.gitter.im';
      c.send(mask, 'PRIVMSG', room.name, ':' + msg.text);
    }.bind(this))
    .catch(function(err) {
...
Adapter.prototype.subscribeToRoom = function(room) {
  ...
  room.on('chatMessages', function(evt) {
    ...
    var text = evt.operation === 'update' ? '[edit] ' + message.text : message.text;
