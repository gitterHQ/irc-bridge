...
// NOTE: wtf isn't this using debug?
var log = console.log;
...
Adapter.prototype.joinRooms = function(channels, key) {
  if (channels) channels.split(',').map(this.joinRoom.bind(this));
};

//
// Function required to get all the users from a room using multiple requests
// as the gitter API doesn't return more than 100 (currently) per request
// This API leaves something to be desired due to the race conditions as we
// query for all users in large rooms. Also, not sure why limit isn't honored
// and is instead capped at 100.
//
function getAllUsers (room) {
  var sanity = 0;
  var skip = 0;
  var userChunks = [];
  function recurse (resolve, reject, notify) {
    var args = { "skip": skip,
                 "limit": 5000}; // maybe it will not be capped someday

    if (sanity++ > 1000) {
      // with current API max of 100 covers 100k users.
      throw "Runnaway recursion > 1000 levels";
    }

    debug("getAllUsers:recurse: skip: " + skip);
    room.users(args)
    .then(function (chunk) {
      if (chunk.length == 0) {
        var allUsers = [].concat.apply([], userChunks);
        resolve(allUsers);
        debug("getAllUsers:resolved");
        return;
      }

      skip += chunk.length;
      userChunks.push(chunk);
      recurse(resolve, reject, notify);
    });
  }
